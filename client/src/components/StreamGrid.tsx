...
import { playbackLabel, remaining, sourceLabel } from '../lib/format';
...
function StreamCard({ stream }: { stream: Stream }) {
  ...
  return (
    <article className="card flex gap-4 p-3.5">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <p className="mt-1 truncate text-xs text-fog-500">
          {stream.you ? 'You · ' : stream.user ? `${stream.user} · ` : ''}
          {stream.player}
        ...
        <div className="mt-auto pt-3">
          ...
          <div className="mt-1.5 flex items-center justify-between text-[11px] text-fog-500">
            ...
            <span className="flex items-center gap-1.5">
              ...
              <span className="chip bg-white/5 text-fog-500">{sourceLabel[stream.source]}</span>
            </span>
          </div>
        </div>
      </div>
